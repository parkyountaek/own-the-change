...
FIXTURE = RESOURCE_ROOT / "assets/demo"
if not FIXTURE.is_dir():
    FIXTURE = RESOURCE_ROOT / "tests/fixtures/host-smoke"
...
def prepare_demo(output=None):
    ...
    if not all((FIXTURE / name).is_file() for name in inputs):
        raise ValueError("Demo fixtures are missing; update the plugin or use a complete source checkout")
...
def main():
    ...
    parser.add_argument("--path-only", action="store_true", help="Print only the new directory for the host agent")

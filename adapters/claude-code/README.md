...
The marketplace template is `../../templates/claude-marketplace.json`. The builder places it beside the complete package under `dist/claude-code/.claude-plugin/marketplace.json`. `python3 scripts/sync_marketplace.py` also generates the repository-root catalog and tracked `plugins/claude-code/own-the-change/` distribution for installation through `/plugin marketplace add parkyountaek/own-the-change`. Use the [release checklist](../../docs/releasing.md) to test catalog validation and cached installation separately. Having an adapter directory doesn't mean it can be installed directly.

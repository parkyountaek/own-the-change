...
The native package also includes three thin checkpoint skills from the top-level `skills/own-change-debrief`, `skills/own-plan-check`, and `skills/own-understanding-check` directories. Their `agents/openai.yaml` files provide picker labels and starter prompts. In Codex CLI, select an action from `/skills` or mention it with `$`; see the [usage map](../../docs/codex-usage.md). These shortcuts route to the same shared entry point and canonical protocol. They are bundled only in Codex; Claude already has named commands. The legacy local installer still links only the generic `own-the-change` skill.

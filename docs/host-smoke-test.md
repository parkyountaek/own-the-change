...
| Choice quality | Inspect each question before selecting, then compare feedback with the actual diff/test evidence | Four distinct plausible content options and uncertainty last; one supported key, unchanged mapping, no immediate key-position repeat or fixed cycle; no answer hinted by wording or length |
| Distractor regression | Start fresh checks on separately scoped whitespace, cache-expiry boundary, and retry-limit changes; do not supply suggested questions or answers | Concrete first question where supported; parallel options with a distinct code/requirement misunderstanding behind every distractor; no unrelated feature used merely to fill a slot. Retain failures as well as successes; a larger option count alone is not a pass |
| Result-based questions | Run a task in which a subagent or reviewer produced process commentary (for example an internal code-review note) before requesting the check | Questions target the final diff, behavior, and test evidence; the review conversation itself is not quizzed unless its point survives as changed code or a recorded remaining risk |
| Human-readable learning | Answer a prediction or related-fix scenario and intentionally select a plausible misconception in a separate session | Everyday phrasing in the conversation's language; feedback explains the specific mistaken boundary without a score, essay demand, or automatic extra question |
| Context reuse | Continue with a number-only reply while relevant input fingerprints remain unchanged | No repeated full-protocol/diff dump or unnecessary test rerun; changed or unverifiable inputs invalidate reuse |
| No-save completion | Request a debrief or check without saving; also test a first-save offer in an unignored target | No record directory or placeholder for no-save; exact tracking/ignore result before a save choice; no ignore-setting changes |
| Bundled setup | Invoke Demo and Doctor from outside Git in an installed copy | Demo uses only a new synthetic target; Doctor performs read-only prerequisite checks; neither silently reads the caller's code or creates a learning record |
